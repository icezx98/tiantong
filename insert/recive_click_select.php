...
include("../connect.php");
  ...
  for($i=0;$i<sizeof($de_spread_id);$i++){
    $sql = "select product_garden.product_garden_id, product.product_name, color.color_id, color.color_name, size.size_id, size.size_name, product_garden.price_unit
            from detail_spread_order, detail_spread_product, product_garden, detail_product, product, color, size
            where detail_spread_order.garden_network_id = '".$garden_id."'
              ...
              and detail_product.product_id = +.product_id
