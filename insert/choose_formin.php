...
<link href="../css/bootstrap.min.css" rel="stylesheet">
 ...
 <script type="text/javascript" src="../js/choose_popup.js"></script>
 <script type="text/javascript" src="../js/choose_formin_select.js"></script>
</head>
<body ng-controller='choose_selectcontroller'>
...
<div class="row">
  ...
  <div class="col-md-7" style="font-size:2em;">
   ...
   <!-- /
	 ...
	 /
    <a href="../choose.php">คัดแยกสินค้า(เพิ่มข้อมูล)</a>
...
<?php
 ...
 $test_id = "select MAX(SUBSTRING(choose_id,4)) as num FROM choose";
 ...
 if($rows){
  $num = $rows['num'];
    ...
    $test_id = $num+1;
    if($test_id < 10){
     $choose_id = "CH000".$test_id;

        }
      elseif ($test_id < 100) {
        $choose_id = "CH00".$test_id;
      }
      elseif ($test_id < 1000) {
        $choose_id = "CH0".$test_id;
      }
      elseif ($test_id < 10000) {
        $choose_id = "CH".$test_id;
      }
    }
...
<table >
  ...
  <input type="text" class="form-control" placeholder="กรอกข้อมูล"  aria-describedby="sizing-addon2" name="choose_id" id='choose_id' readonly="readonly" style="width:200px" value="<?php echo $choose_id;?>">
</div>
</td>
</tr>
...
<!-- <select ng-model="recive" ng-change="rec()" class="form-control" name="garden_id" style="width:227px" oninvalid="setCustomValidity('กรุณาเลือกข้อมูล')" oninput="setCustomValidity('')" required> -->
     <select ng-model="cho" ng-change="choose()" class="form-control" name="garden_id" style="width:227px" oninvalid="setCustomValidity('กรุณาเลือกข้อมูล')" oninput="setCustomValidity('')" required>
  <?php
     $sql = "select detail_spread_order.garden_network_id, garden_network.garden_network_name
             from detail_spread_order, detail_spread_product, garden_network, recive, detail_recive
             ...
             and detail_spread_product.status = '1'
             and recive.choose_status = '0'
             and recive.recive_id = detail_recive.recive_id
             and detail_recive.product_id = detail_spread_product.product_garden_id
 ...
 ?>
    ...
    <tr>
  <td style="width:200px"><div class="col-md-9 col-md-push-5">วันที่คัดแยกสินค้า</div><div class="col-md-2 col-md-push-2"><b>&nbsp;&nbsp;&nbsp;*</b></div></td>
  <td style="width:200px">
<div class="input-group">
  <span class="input-group-addon" id="sizing-addon2"></span>
  <?php
...
<!-- <div class="row">
  ...
  <!-- <tr ng-repeat="ice in result track by $index">
      <td class='td' align="center"><label><input type='checkbox' ng-model='check' ng-true-value='1' ng-false-value='null'></label></td>
      <td ng-click="click(ice[1])" ng-repeat="nic in ice track by $index" align="center">{{nic}}</td> 
  </tr> -->
  <tr ng-repeat="ice in result track by $index"  >
      <td class='td' align="center"><label><input type='checkbox' ng-model="check2[$index]" ng-change='check2(ice[0])'></label></td>
      <!-- <td align="center" ng-click="cl2(ice[1],$index)">{{ice[0]}}</td> -->
      <td align="center" ng-click="cl2(ice[0],ice[1],$index)">{{ice[1]}}</td>
      <td align="center" ng-click="cl2(ice[0],ice[1],$index)">{{ice[2]}}</td>
  </tr>
</table>
  </form>
  </div>
