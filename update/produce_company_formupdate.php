...
<div class="row">
  ...
  <a href="../product_data.php">ข้อมูลสินค้า</a>
   / 
    <a href="../product_company.php">สินค้าของบริษัท(แก้ไขข้อมูล)</a>
...
<?php
 ...
 $sql = "select * from product_company where product_company_id = '".$_GET["id"]."'" ;
 ...
 $product_company_id=$data[0];
 $produce_unit=$data[1];
 $Date_newprice=$data[2];
 $produce_id=$data[3];
 $company_id=$data[4];
 ...
 if( !empty($_GET['o'])){
    ...
    $produce_id = "";
    $company_id="";
    if( !empty($_GET['product_company_id']))
        {
          $product_company_id = $_GET['product_company_id'];
        }
    if( !empty($_GET['price_unit']))
        {
          $produce_unit = $_GET['price_unit'];
        }
    if( !empty($_GET['Date_newprice']))
        {
          $Date_newprice = $_GET['Date_newprice'];
        }
...
<div class="clearfix hidden-xs">
    <form method="post" action="product_company_update.php">
       <input type='hidden' name='produce_id' value='{{id}}'>
<table >
  <tr >
  <td style="width:200px"><div class="col-md-9 col-md-push-5">รหัสสินค้าของบริษัท</div><div class="col-md-2 col-md-push-2"><b>&nbsp;&nbsp;&nbsp;*</b></div></td>
...
<tr>
  ...
  <td>
    ...
    <select class="form-control" name="company_id" style="width:227px" oninvalid="setCustomValidity('กรุณาเลือกข้อมูล')" oninput="setCustomValidity('')" required>
    <?php
...
<div class="input-group">
  ...
  <input type="text" onKeyUp="var regExp = /^[0-9,'.']*$/;if(!regExp.test(this.value)){this.value = '';}" style="text-align: right;width: 200px;" class="form-control" placeholder="0.00" aria-describedby="sizing-addon2" name="price_unit" OnChange="JavaScript:chkNum(this)" style="width:200px" value="<?php echo  number_format($produce_unit, 2, '.', ',');?>" >
</div>
  </td>
    </tr>
    <tr>
  <td style="width:200px"><div class="col-md-9 col-md-push-5">วันที่เริ่มใช้ราคาใหม่</div><div class="col-md-2 col-md-push-2"><b>&nbsp;&nbsp;&nbsp;*</b></div></td>
...
<div class="input-group">
  ...
  <?php
    ...
    ?>
  <input type="text" text-align="center" class="form-control"  aria-describedby="sizing-addon2" name="dateInput"   style="width:200px" value="<?php echo $date;?>" readonly="readonly">
</div>
  </td>
    </tr>
  <tr>
...
<div class="row">
  ...
  <div class="col-xs-8 col-md-6"><input type="submit" value="เเก้ไข" style="width:80px">&nbsp;<input type="reset" value="ยกเลิก" style="desvice-widthh:80px" ONCLICK="window.location.href='../product_company.php'"></td></div>
</div>
...
<tr>
 <td>
  ...
  <div class="col-md-9 col-md-push-4">
    <select  class="form-control" ng-model="unitSelect" ng-change="getId()" style="width:230px" oninvalid="setCustomValidity('กรุณาเลือกข้อมูล')" oninput="setCustomValidity('')" required>
                                <option value="" selected>คลิกเลือกหน่วยนับ</option>
                                <option value="{{$index}}" ng-model="unitSelect" ng-repeat="data in unit"> {{data[1]}} </option>
                            </select>
                            </div></div>
  ...
  <div class="col-md-9 col-md-push-4">
    <select class="form-control" name="company_id" style="width:230px" oninvalid="setCustomValidity('กรุณาเลือกข้อมูล')" oninput="setCustomValidity('')" required>
    <?php
...
<tr>
  ...
  <input type="text" onKeyUp="var regExp = /^[0-9,'.']*$/;if(!regExp.test(this.value)){this.value = '';}" style="text-align: right;width: 230px;" class="form-control" placeholder="0.00" aria-describedby="sizing-addon2" name="price_unit" OnChange="JavaScript:chkNum(this)" style="width:200px" value="<?php echo  number_format($produce_unit, 2, '.', ',');?>" >
</div></div>
  </td>
    </tr>
    <tr>
  <td >
     <div class="input-group">
  <div class="col-md-9 col-md-push-4">วันที่เริ่มใช้ราคาใหม่<b>*</b></div>
  <div class="col-md-9 col-md-push-4">
    <?php
     $date = date("Y-m-d");
    ?>
  <input type="text" text-align="center" class="form-control"  aria-describedby="sizing-addon2" name="dateInput" style="width:230px" value="<?php echo $Dadatete_newprice;?>" readonly="readonly">
</div></div>
  </td>
    </tr>
  <tr>
  ...
  <center><input type="submit" value="เเก้ไข" style="width:80px">&nbsp;<input type="reset" value="ยกเลิก" style="desvice-widthh:80px" ONCLICK="window.location.href='../product_company.php'"></td></center>
